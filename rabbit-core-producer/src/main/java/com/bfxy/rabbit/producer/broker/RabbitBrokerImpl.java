...
/**
 * @author kongdz
 * @notes $RabbitBrokerImpl 真正的发送不同类型的消息实现类
...
public class RabbitBrokerImpl implements RabbitBroker {
	...
	@Autowired
	private RabbitTemplateContainer mRabbitTemplateContainer;
	...
	public void reliantSend(Message message) {
		...
		BrokerMessage bm = mMessageStoreService.selectByMessageId(message.getMessageId());
		if(bm == null) {
			//1.在broker_message表里创建消息记录
			Date now = new Date();
			...
			mMessageStoreService.insert(brokerMessage);
		}
		//2. 执行的发送消息的方法
		sendKernel(message);
	}
	...
	private void sendKernel(Message message) {
		AsyncBaseQueue.submit((Runnable) () -> {
			CorrelationData correlationData =
					new CorrelationData(String.format("%s#%s#%s", message.getMessageId(), System.currentTimeMillis(),
							message.getMessageType()));
			String topic = message.getTopic();
			String routingKey = message.getRoutingKey();
			RabbitTemplate rabbitTemplate = mRabbitTemplateContainer.getTemplate(message);
